...
public:
  SwiftAllPayloadEnumDescriptor(swift::ASTContext *ast,
    ...
    for (auto enum_case : elements_with_payload) {
      ...
      if (arg_type) {
        case_type = SwiftASTContext::GetCompilerType(
...
SwiftEnumDescriptor *SwiftASTContext::GetCachedEnumInfo(void *type) {
  ...
  if (type) {
    ...
    if (!SwiftASTContext::IsFullyRealized(
            SwiftASTContext::GetCompilerType(swift_can_type)))
      return nullptr;
...
SwiftASTContext::GetTypeFromMangledTypename(ConstString mangled_typename,
  ...
  if (found_type) {
    ...
    return SwiftASTContext::GetCompilerType(found_type);
  }
  ...
  if (found_type) {
    ...
    CompilerType result_type =
        SwiftASTContext::GetCompilerType(found_type);
    assert(&found_type->getASTContext() == ast_ctx);
...
CompilerType SwiftASTContext::GetAnyObjectType() {
  VALID_OR_RETURN(CompilerType());
  return SwiftASTContext::GetCompilerType(
...
CompilerType SwiftASTContext::GetVoidFunctionType() {
  ...
  if (!m_void_function_type) {
    ...
    m_void_function_type = SwiftASTContext::GetCompilerType(
        swift::FunctionType::get({}, empty_tuple_type));
  }
  ...
  if (decl) {
    ...
    case swift::DeclKind::TypeAlias: {
      ...
      if (alias_decl->hasInterfaceType()) {
        ...
        return SwiftASTContext::GetCompilerType(swift_type);
      }
      break;
    }
...
CompilerType SwiftASTContext::ImportType(CompilerType &type, Status &error) {
  ...
  if (mangled_name) {
    ...
    if (our_type_base)
      return SwiftASTContext::GetCompilerType(our_type_base);
    else {
      Status error;
...
CompilerType SwiftASTContext::GetErrorType() {
  ...
  if (swift_ctx) {
    ...
    if (error_type_decl) {
      ...
      return SwiftASTContext::GetCompilerType(error_type);
    }
  }
  return {};
}
...
uint32_t SwiftASTContext::GetPointerByteSize() {
  ...
  if (m_pointer_byte_size == 0)
    m_pointer_byte_size = SwiftASTContext::GetCompilerType(
...
bool SwiftASTContext::IsArrayType(void *type, CompilerType *element_type_ptr,
  ...
  if (struct_type) {
    ...
    if (element_type_ptr)
      *element_type_ptr =
          SwiftASTContext::GetCompilerType(args[0].getPointer());
    return true;
  }
...
SwiftASTContext::GetReferentType(const CompilerType &compiler_type) {
  ...
  if (compiler_type.IsValid() &&
    ...
    return SwiftASTContext::GetCompilerType(ref_type);
  }
  ...
  if (auto ast =
    ...
    if (auto superclass = layout.explicitSuperclass) {
      protocol_info.m_superclass =
          SwiftASTContext::GetCompilerType(superclass.getPointer());
    }
...
GetArchetypeNames(swift::Type type, swift::ASTContext &ast_ctx,
  ...
  assert(&type->getASTContext() == &ast_ctx);
  if (!sc)
    return dict;
  ...
  type.visit([&](swift::Type swift_type) {
    if (!swift_type->isTypeParameter() ||
        dict.count(swift_type->getCanonicalType()))
      return;
    auto *param = swift_type->getAs<swift::GenericTypeParamType>();
    auto it = names.find({param->getDepth(), param->getIndex()});
    if (it != names.end()) {
      swift::Identifier ident = ast_ctx.getIdentifier(it->second);
      dict.insert({swift_type->getCanonicalType(), ident});
    }
  });
  return dict;
}
...
SwiftASTContext::GetTypeInfo(void *type,
  ...
  case swift::TypeKind::WeakStorage:
    swift_flags |= SwiftASTContext::GetCompilerType(
...
CompilerType SwiftASTContext::GetInstanceType(void *type) {
  ...
  return SwiftASTContext::GetCompilerType(GetSwiftType(type));
}
        ...
        case swift::DeclKind::Func: {
          if (idx == 0) {
            ...
            if (abstract_func_decl) {
              ...
              result_type = SwiftASTContext::GetCompilerType(
...
uint32_t SwiftASTContext::GetNumChildren(void *type,
  ...
  case swift::TypeKind::ProtocolComposition: {
    ...
    if (!GetProtocolTypeInfo(
            SwiftASTContext::GetCompilerType(GetSwiftType(type)),
            protocol_info))
      break;
  ...
  case swift::TypeKind::LValue: {
    ...
    uint32_t num_pointee_children =
        SwiftASTContext::GetCompilerType(deref_type)
            .GetNumChildren(omit_empty_base_classes, exe_ctx);
...
SwiftASTContext::GetDirectBaseClassAtIndex(void *opaque_type, size_t idx,
  ...
  if (opaque_type) {
    ...
    if (class_decl) {
      ...
      if (base_class_type)
        return SwiftASTContext::GetCompilerType(base_class_type);
    }
  }
  return {};
}
...
GetExistentialTypeChild(swift::ASTContext *swift_ast_ctx, CompilerType type,
  ...
  if (idx < protocol_info.m_num_payload_words) {
    ...
    return {SwiftASTContext::GetCompilerType(raw_pointer.getPointer()),
            std::move(name)};
  }
  ...
  if (idx == 0 && protocol_info.m_is_class_only) {
    ...
    if (protocol_info.m_superclass) {
      ...
      auto raw_pointer = swift_ast_ctx->TheRawPointerType;
      class_type =
          SwiftASTContext::GetCompilerType(raw_pointer.getPointer());
    }
  ...
  if (idx && idx == protocol_info.m_num_payload_words) {
    ...
    return {SwiftASTContext::GetCompilerType(any_metatype),
            "instance_type"};
  }
...
CompilerType SwiftASTContext::GetFieldAtIndex(void *type, size_t idx,
  ...
  case swift::TypeKind::Tuple: {
    ...
    return SwiftASTContext::GetCompilerType(child.getType().getPointer());
  }
  ...
  case swift::TypeKind::BoundGenericClass: {
    ...
    if (class_decl->hasSuperclass()) {
      if (idx == 0) {
        ...
        CompilerType superclass_type = SwiftASTContext::GetCompilerType(
  ...
  case swift::TypeKind::BoundGenericStruct: {
    ...
    return SwiftASTContext::GetCompilerType(child_swift_type.getPointer());
  }
...
bool SwiftASTContext::IsNonTriviallyManagedReferenceType(
  ...
  if (auto ast =
    ...
    case swift::TypeKind::UnmanagedStorage: {
      ...
      if (underlying_type)
        *underlying_type = SwiftASTContext::GetCompilerType(
...
CompilerType SwiftASTContext::GetChildCompilerTypeAtIndex(
  ...
  case swift::TypeKind::Tuple: {
    ...
    CompilerType child_type =
        SwiftASTContext::GetCompilerType(child.getType().getPointer());
  ...
  case swift::TypeKind::BoundGenericStruct: {
    ...
    CompilerType child_type =
        SwiftASTContext::GetCompilerType(child_swift_type.getPointer());
    ...
    case swift::TypeKind::Module: {
      ...
      for (swift::Decl *decl : decls) {
        ...
        if (kind >= swift::DeclKind::First_TypeDecl &&
          ...
          if (type_decl) {
            CompilerType clang_type = SwiftASTContext::GetCompilerType(
...
CompilerType SwiftASTContext::GetCompilerType(swift::Type swift_type) {
  return CompilerType(
      SwiftASTContext::GetSwiftASTContext(&swift_type->getASTContext()),
      swift_type.getPointer());
}
